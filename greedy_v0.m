function fit=greedy_v0(sensor_link_fog)%传感器位置、候选雾节点位置传进来的参数是一组x,y坐标
...
fog_available= fog_available.fog_available;
...
% 10个选中雾设备的坐标,找到10个雾设备的坐标，有重复就不要
fog_selected_location = zeros(10,2);

for i = 1:10
    fog_selected_location(i,:) = fog_available(fog_selected_number(i),:);
end
...
fit = zeros(1,100);
for i =1: 100
    fit(i) = fog_selected_number(index(i));
end
